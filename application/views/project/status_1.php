<?php 
	$crud->columns('name', 'site_id', 'consultant_id', 'started_date', 'close_date', 'contract_model_id','phase_id', 'file');
	$crud->set_relation('site_id','site','name_ar');
	$crud->set_relation('consultant_id','consultant','name');
		
	$crud->set_relation('phase_id','phase','name');
	$crud->set_relation('contract_model_id','contract_model','name');
	//$crud->where('project_phase_id', 1);
	...
	$crud->display_as('name', $this->lang->line('project_name'))
			...
			->display_as('phase_id', $this->lang->line('project_phase'))
			->display_as('file', $this->lang->line('file'));
	//fields will be displayed in add/edit form
	$crud->fields('name', 'site_id', 'consultant_id', 'started_date', 'close_date','contract_model_id', 'file');
